...
While the results may be viewed as a step in a right direction, it should be kept in mind that accelerometer-only sleep stage analysis (for example with a smartphone) may yield similar scores[^SOURCE3].
...
Just to check how (already not if) NeuroOn's results are different from a "coin toss", here's a bootstrapped analysis from the second night:
...
The most users may be interested in two questions:
 ...
 - Will NeuroOn wake me up when it has an opportunity to?
 - Won't NeuroOn wake me up when it shouldn't?
...
<!-- Even knowing that we will not be able to conclude if it is possible to create a real-time sleep staging algorithm based on NeuroOn signal, we decided to check how much data we can extract from it.

To do that, we resorted to spectral analysis, studying delta power in NeuroOn's single-channel signal.

[understandable analysis here] -->

### Why use EEG?

<!-- Comparison to accelerometer-based analysis [MOAR HERE] -->

### Contrast with current claims

Reading NeuroOn's *Final* Press Release:
...
I consider startups and Kickstarter campaigns like IntelClinic's NeuroOn deeply harmful. Promising a product that you haven't even checked is possible to build is just plain lying. There's nothing brave in it and it has nothing to do with entrepreneurship . Spending more money on marketing campaigns than actual, peer-reviewed research is just scamming.
